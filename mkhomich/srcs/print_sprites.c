...
/*   Updated: 2019/12/28 19:19:53 by nlavrine         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_aim(t_doom *doom)
{
	...
	x = doom->x_aim;
	y = doom->y_aim - (size / 2);
	while (y <= doom->surface->h * 0.62 + (size / 2))
	{
		put_pixel(doom, x, y, 0xFFFFFF);
		y++;
	}
...
void	print_weapons(t_doom *doom)
{
	if (doom->play[doom->n_play].weapons == 6 && doom->move.altern == 0)
		print_sprite(doom, doom->surface->w * 0.65, doom->surface->h, 6, 0, 0);
	else if (doom->play[doom->n_play].weapons == 6 && doom->move.altern)
	{
		print_sprite(doom, doom->surface->w * 0.65, doom->surface->h, 16, 0, 0);
		doom->sp[16].count++;
	}
	if (doom->play[doom->n_play].weapons == 7)
		print_sprite(doom, doom->surface->w * 0.82, doom->surface->h, 7, 0, 0);
	if (doom->play[doom->n_play].weapons == 8)
		print_sprite(doom, doom->x_aim, doom->surface->h, 8, 0, 0);
}
...
void	print_hud(t_doom *doom)
{
	...
	{
		print_sprite(doom, doom->surface->w * 0.05, doom->surface->h * 0.95, 0, 0, 0);
		print_sprite(doom, doom->surface->w * 0.05, doom->surface->h * 0.98, 1, 0, 0);
		print_sprite(doom, doom->surface->w * 0.15, doom->surface->h * 0.96, 2, 0, 0);
	}
	if ((doom->sp[4].count = doom->play[doom->n_play].armor) > 0)
	{
		print_sprite(doom, doom->surface->w * 0.3, doom->surface->h * 0.98, 3, 0, 0);
		print_sprite(doom, doom->surface->w * 0.43, doom->surface->h * 0.96, 4, 0, 0);
	}
	doom->sp[5].count = doom->play[doom->n_play].shot - 1;
	if (doom->play[doom->n_play].shot)
		print_sprite(doom, doom->surface->w * 0.91, doom->surface->h * 0.97, 5, 0, 0);
}

void	check_mob_sp(t_doom *doom, int nb)
{
	if (doom->play[nb].sp == 9)
		doom->sp[9].zoom = (1 / doom->play[nb].t.tz2) * 4;
	if (doom->play[nb].sp == 10)
		doom->sp[10].zoom = (1 / doom->play[nb].t.tz2) * 30;
	if (doom->play[nb].sp == 11)
		doom->sp[11].zoom = (1 / doom->play[nb].t.tz2) * 30;
}
